...
fn compute_antinodes(pair: ((i32, i32), (i32, i32)), map: Array2<char>) -> Vec<(i32, i32)> {
    ...
    for i in 0..map.nrows() {
        let antinode = (p1.0 - i as i32 * dy, p1.1 - i as i32 * dx);
        if antinode.0 >= 0 && antinode.1 >= 0 && antinode.0 < map.nrows() as i32 && antinode.1 < map.ncols() as i32 {
    ...
    for i in 0..map.nrows() {
        let antinode = (p2.0 + i as i32 * dy, p2.1 + i as i32 * dx);
        if antinode.0 >= 0 && antinode.1 >= 0 && antinode.0 < map.nrows() as i32 && antinode.1 < map.ncols() as i32 {
            antinodes.push(antinode);
        } else {
            break;
...
fn extract_antinodes(nodes: HashMap<char, Vec<(i32, i32)>>, map: Array2<char>) -> Vec<(i32, i32)> {
    let mut all_antinodes = vec![];
    for (_, points) in nodes {
        let pairs = get_unique_pairs(&points);
        for pair in pairs {
            let antinodes = compute_antinodes(pair, map.clone());
...
fn main() -> io::Result<()> {
    ...
    let antinodes = extract_antinodes(nodes, map.clone());
    let unique_antinodes = filter_unique_antinodes(antinodes, map.clone());
